...
-   列の名前や型を指定しやすい
-   指定した列だけ読み込むこともできる
-   生data.frameより安全な [tibble](#tibble) として返してくれる
...
`readr::write_*()` のデフォルトはそれよりだいぶマシだが、
欠損値を空欄ではなく `NA` にしてしまうことだけ要注意。
空欄にするには毎回 `na = ""` が必要。
...
:   文字列を整数型として解釈して返す。
    ...
    [1]  42 129  NA  NA
    ```
    標準の `as.integer()` が1ではなく129を返すということは、
    一旦実数型で読み取ったあと小数点以下を切り捨ててるっぽい。
...
`read::read_*()` で読み込んだばかりのtibbleは各列の型情報を含んだ
`spec_tbl_df` という特殊なサブクラスになっている。
