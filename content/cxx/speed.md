...
実行時の計算を減らすことができる。
C++11以降では明示的に `constexpr` 修飾することができるので積極的に使うべし。
テンプレートメタプログラミングという手もある。
定数同士の四則演算などは近くにまとめておけばコンパイラが計算してくれるはず。
...
ということを示すだけで、普通の変数を渡して普通の関数のようにも使える。
...
(=インライン展開) でそのコストを無くせる。
実行ファイルのサイズやコンパイル時間とのトレードオフはあるけど。
...
インライン展開に必要でも十分でもない。
`-O2` 以上の速度重視オプションで
`-finline-small-functions` とか `-finline-functions`
のようなオプションをオンにする。
...
ただし C++ Core Guidelines によれば、
[F.60: Prefer T* over T& when “no argument” is a valid option
](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#Rf-ptr-ref)
とのことで、むしろ
`nullptr` で値なしを表現する必要が無いなら普通に参照のほうがシンプルに書けるし生成されるコードも良くなるかも、とのこと。
